...
class FormBuilderFrameworkUikit extends FormBuilderFramework {
	...
	public function load() {
		...
		// $config->styles->append($config->urls->FormBuilder . 'frameworks/FormBuilderFrameworkUikit.css');
	...
	public function hookBeforeRenderReady($event) {
		...
		foreach($inputfields->getAll() as $in) {
			...
			if($in instanceof InputfieldCheckbox 
				|| $ifc === 'InputfieldRadios'
				|| $in instanceof InputfieldRadios 
				|| $in instanceof InputfieldCheckboxes) continue;
